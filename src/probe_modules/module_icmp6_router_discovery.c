...
// Probe module to scan for routers (core/periphery) using icmp6 echo scans
// This module sends the original destination IP address within the ICMP6 payload section
// When scanning random subnets or the subnet router anycast address (::0) a router replies with either 
// an error message or an echo reply message -- revealing its real IP address
// The payload gets mirrored and is included in the reply message
// The module is build up on the icmp6 echotime module
...
probe_module_t module_icmp6_router_discovery;
...
static int icmp6_router_discovery_prepare_packet(void *buf, macaddr_t *src, macaddr_t *gw,
				    UNUSED void *arg_ptr)
{
		...
		__attribute__((unused)) struct timespec ts)
{
	...
	/*if (memcmp(&payload->dst_init, &ip6_hdr->ip6_src, sizeof(struct in6_addr))==0){
		printf("IPv6 Address Init is equal to dst addr: %s\n",
		   inet_ntop(AF_INET6, &(payload->dst_init), (char[INET6_ADDRSTRLEN]){0}, INET6_ADDRSTRLEN));
	}
	if (memcmp(&payload->secondary_ip, &ip6_hdr->ip6_src, sizeof(struct in6_addr))==0){
		printf("IPv6 Address Init is equal to src addr: %s\n",
		   inet_ntop(AF_INET6, &(payload->dst_init), (char[INET6_ADDRSTRLEN]){0}, INET6_ADDRSTRLEN));
	}
		printf("IPv6 Init: %s\n",
		   inet_ntop(AF_INET6, &(payload->dst_init), (char[INET6_ADDRSTRLEN]){0}, INET6_ADDRSTRLEN));
		printf("IPv6 Src: %s\n",
		   inet_ntop(AF_INET6, &(ip6_hdr->ip6_src), (char[INET6_ADDRSTRLEN]){0}, INET6_ADDRSTRLEN));
		printf("IPv6 secondary ip: %s\n",
		   inet_ntop(AF_INET6, &(payload->secondary_ip), (char[INET6_ADDRSTRLEN]){0}, INET6_ADDRSTRLEN));
		   */
	if (icmp6_hdr->icmp6_type == ICMP6_ECHO_REPLY) {
		...
		fs_add_string(fs, "initial-ip", make_ipv6_str(&(payload->secondary_ip)), 1);
		...
		if (zconf.all_replies){
			...
			if (memcmp(&payload->secondary_ip, &ip6_inner_hdr->ip6_src, sizeof(struct in6_addr))==0){
				fs_add_uint64(fs, "success", 0);
			}else{
				fs_add_uint64(fs, "success", 1);
			}
...
probe_module_t module_icmp6_router_discovery = {
	...
	.prepare_packet = &icmp6_router_discovery_prepare_packet,
